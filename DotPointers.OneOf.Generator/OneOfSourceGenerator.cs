...
namespace DotPointers.OneOf.Generator
{
	public static class OneOfSourceGenerator
	{
		...
		public static string GenerateUnitySource(GenerationModel model)
		{
			string enumName = $"{model.FullName}.OneOf{model.Name}";
			bool hasNamespace = !string.IsNullOrEmpty(model.Namespace);

			string wrapperName = $"{model.Name}UnityWrapper";
			string genericArgs = model.FullName.Contains("<") ? model.FullName.Substring(model.FullName.IndexOf("<")) : "";
			string wrapperFullName = wrapperName + genericArgs;

			string safeDrawerName = wrapperName + genericArgs.Replace("<", "_").Replace(">", "_").Replace(",", "_").Replace(" ", "");

			var sb = new IndentedWriter();

			sb.AppendLine($"// <auto-generated/> by {nameof(OneOfSourceGenerator)} for Unity support");
			sb.AppendLine("#nullable enable");
			sb.AppendLine("using System;");
			...
			sb.AppendLine($"partial {model.TypeKind} {model.FullName}");
			using (sb.EnterScope())
			{
				sb.AppendLine("[Serializable]");
				sb.AppendLine($"public struct {wrapperFullName}");
				using (sb.EnterScope(false))
				{
					sb.AppendLine($"[SerializeField] private {enumName} _kind;");
					for (int i = 0; i < model.TypeArgs.Length; i++)
					{
						if (model.TypeArgs[i].IsVoid) { continue; }
						sb.AppendLine($"[SerializeField] private {model.TypeArgs[i].FullName} _v{i};");
					}

					sb.AppendLine();

					sb.AppendLine($"public {model.FullName} Value");
					using (sb.EnterScope())
					{
						sb.AppendLine("get");
						using (sb.EnterScope())
						{
							...
							for (int i = 0; i < model.TypeArgs.Length; i++)
							{
								if (model.TypeArgs[i].IsVoid)
								{
									sb.AppendLine($"{enumName}.{model.FieldNames[i]} => {model.FullName}.From{model.FieldNames[i]}(),");
								}
								else
								{
									sb.AppendLine($"{enumName}.{model.FieldNames[i]} => {model.FullName}.From{model.FieldNames[i]}(_v{i}),");
								}
							}

							if (model.AllowEmpty)
							{
								sb.AppendLine($"_ => {model.FullName}.Empty");
							}
							else
							{
								sb.AppendLine($"_ => {model.FullName}.From{model.FieldNames[0]}(default!)");
							}
						...
						using (sb.EnterScope(false))
						{
							...
							{
								if (model.TypeArgs[i].IsVoid) { continue; }
								sb.AppendLine($"_v{i} = default!;");
							}

							sb.AppendLine("_kind = value.Kind;");
							for (int i = 0; i < model.TypeArgs.Length; i++)
							{
								if (model.TypeArgs[i].IsVoid) { continue; }
								sb.AppendLine($"if (value.Is{model.FieldNames[i]}) {{ _v{i} = value.{model.FieldNames[i]}; }}");
							}
						}
					}

					sb.AppendLine($"public static implicit operator {model.FullName}({wrapperFullName} wrapper) => wrapper.Value;");
					sb.AppendLine($"public static implicit operator {wrapperFullName}({model.FullName} val) => new {wrapperFullName} {{ Value = val }};");

					sb.AppendLine("#if UNITY_EDITOR");
					sb.AppendLine($"[CustomPropertyDrawer(typeof({wrapperFullName}))]");
					sb.AppendLine($"internal class {safeDrawerName}PropertyDrawer : PropertyDrawer");
					using (sb.EnterScope())
					{
						sb.AppendLine("public override float GetPropertyHeight(SerializedProperty property, GUIContent label)");
						using (sb.EnterScope())
						{
							sb.AppendLine("var kindProp = property.FindPropertyRelative(\"_kind\");");
							sb.AppendLine("if (kindProp == null) { return EditorGUIUtility.singleLineHeight; }");
							sb.AppendLine();
							sb.AppendLine("float height = EditorGUIUtility.singleLineHeight;");
							sb.AppendLine("switch (kindProp.intValue)");
							using (sb.EnterScope())
							{
								for (int i = 0; i < model.TypeArgs.Length; i++)
								{
									...
									sb.AppendLine($"case {i + 1}: // {model.FieldNames[i]}");
									using (sb.EnterScope(false))
									{
										sb.AppendLine($"var valProp = property.FindPropertyRelative(\"_v{i}\");");
										sb.AppendLine("if (valProp != null) height += EditorGUIUtility.standardVerticalSpacing + EditorGUI.GetPropertyHeight(valProp, true);");
										sb.AppendLine("break;");
									}
								}
							}
						...
						using (sb.EnterScope())
						{
							...
							sb.AppendLine();
							sb.AppendLine("var kindProp = property.FindPropertyRelative(\"_kind\");");
							sb.AppendLine("if (kindProp == null)");
							using (sb.EnterScope())
							{
								sb.AppendLine("EditorGUI.LabelField(position, label.text, \"Drawer Error: _kind not found\");");
								sb.AppendLine("EditorGUI.EndProperty();");
								sb.AppendLine("return;");
							}

							sb.AppendLine("var kindRect = new Rect(position.x, position.y, position.width, EditorGUIUtility.singleLineHeight);");

							sb.AppendLine("EditorGUI.BeginChangeCheck();");
							sb.AppendLine("int oldKind = kindProp.intValue;");
							sb.AppendLine("EditorGUI.PropertyField(kindRect, kindProp, label);");
							sb.AppendLine("if (EditorGUI.EndChangeCheck() && kindProp.intValue != oldKind)");
							using (sb.EnterScope())
							{
								for (int i = 0; i < model.TypeArgs.Length; i++)
								{
									if (model.TypeArgs[i].IsVoid) { continue; }
									sb.AppendLine($"var p{i} = property.FindPropertyRelative(\"_v{i}\");");
									sb.AppendLine($"if (p{i} != null) {{ ResetPropertyValue(p{i}); }}");
								}
							}
							...
							using (sb.EnterScope())
							{
								for (int i = 0; i < model.TypeArgs.Length; i++)
								{
									...
									{
										sb.AppendLine($"var valProp = property.FindPropertyRelative(\"_v{i}\");");
										sb.AppendLine("if (valProp != null)");
										using (sb.EnterScope())
										{
											sb.AppendLine("var valRect = new Rect(position.x, position.y + EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing, position.width, EditorGUI.GetPropertyHeight(valProp, true));");
											sb.AppendLine("EditorGUI.PropertyField(valRect, valProp, new GUIContent(\"Value\"), true);");
										}
										sb.AppendLine("break;");
									}
								}
						...
						sb.AppendLine();
						sb.AppendLine("private void ResetPropertyValue(SerializedProperty prop)");
						using (sb.EnterScope(false))
						{
							sb.AppendLine("switch (prop.propertyType)");
							using (sb.EnterScope(false))
							{
								sb.AppendLine("case SerializedPropertyType.ObjectReference: prop.objectReferenceValue = null; break;");
								sb.AppendLine("case SerializedPropertyType.ManagedReference: prop.managedReferenceValue = null; break;");
								sb.AppendLine("case SerializedPropertyType.String: prop.stringValue = string.Empty; break;");
								sb.AppendLine("case SerializedPropertyType.Integer: prop.intValue = 0; break;");
								sb.AppendLine("case SerializedPropertyType.Boolean: prop.boolValue = false; break;");
								sb.AppendLine("case SerializedPropertyType.Float: prop.floatValue = 0f; break;");
								sb.AppendLine("case SerializedPropertyType.Color: prop.colorValue = Color.clear; break;");
								sb.AppendLine("case SerializedPropertyType.Enum: prop.enumValueIndex = 0; break;");
								sb.AppendLine("case SerializedPropertyType.Vector2: prop.vector2Value = Vector2.zero; break;");
								sb.AppendLine("case SerializedPropertyType.Vector3: prop.vector3Value = Vector3.zero; break;");
								sb.AppendLine("case SerializedPropertyType.Rect: prop.rectValue = Rect.zero; break;");
								sb.AppendLine("case SerializedPropertyType.Generic:");
								using (sb.EnterScope(false))
								{
									sb.AppendLine("if (prop.isArray)");
									using (sb.EnterScope())
									{
										sb.AppendLine("prop.ClearArray();");
									}
									sb.AppendLine("else");
									using (sb.EnterScope())
									{
										sb.AppendLine("var endProperty = prop.GetEndProperty();");
										sb.AppendLine("while (prop.NextVisible(true) && !SerializedProperty.EqualContents(prop, endProperty))");
										using (sb.EnterScope(false))
										{
											sb.AppendLine("ResetPropertyValue(prop);");
										}
									}
									sb.AppendLine("break;");
								}
